...
| **Ключ Amplitude** | ⛔ **`extra.amplitudeApiKey` пустой — НИ ОДНО событие не уходит** |

`initAmplitude` вызывается под `if (amplitudeApiKey)`. Пока строка пустая,
провайдер не устанавливается, а `track()` молча выбрасывает событие. Это
единственное, что отделяет нас от рабочей аналитики.
