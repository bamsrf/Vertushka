...
> ✅ **Сверено с прод-базой 2026-08-23.** Аккаунт `vertushka_demo`
> (review@vinyl-vertushka.store) существует с 2026-08-02, активен. Пароль был
> утерян → **сброшен 2026-08-23** через `hash_password()` самого бэкенда;
> новый пароль передан владельцу, нигде в репозитории не хранится — вписать
> в ASC → Sign-In Information и в менеджер паролей. Вход проверен через
> публичный API: токен выдаётся, **код из почты не требуется**.

- [x] Email: `review@vinyl-vertushka.store` — аккаунт существует, активен
- [x] Регистрация через email+пароль — да, вход по паролю работает
- [x] Наполнить коллекцию — **30 пластинок** (норма 15–20)
- [x] Вишлист — **5 позиций**
- [x] Публичный профиль, bio — заполнено
- [x] Ачивки — **86 открыто**
- [ ] ⚠️ **Диалогов: 0.** Единственный незакрытый пункт — отправить 1–2
      сообщения между демо-аккаунтом и основным, чтобы ревьюер увидел чат
      не пустым (и report/block в действии)
- [x] Логин работает, verify-code не требуется — проверено 2026-08-23 живым запросом
...
| **Зеркалирование обложек в свой S3** ([COVERS_S3_IMGPROXY_MILESTONE.md](../covers/COVERS_S3_IMGPROXY_MILESTONE.md)) | ⚠️ **это уже не показ, а хранение и раздача копий** — то, что Discogs называет mass re-distribution |
...
(M7, [PLAN_MONETIZATION.md](../product/PLAN_MONETIZATION.md)) — появляется выручка, и
