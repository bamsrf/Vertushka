...
### WS4 — EAS-профили, build-properties, первый prebuild (B9, M9, M10, L1) ✅ 2026-09-08 (#168)
...
### WS7 — QA на железе (M4, M5, L3, L7, L8) 🟡 первый прогон на эмуляторе 2026-09-08

**Итог первого прогона** (pixel8_api36, Android 16, dev-client, аккаунт владельца): 14 из 15 сценариев, 136 скриншотов. ✅ поиск/карточка/артист, коллекция и вишлист, ручное добавление с клавиатурой, OptionsSheet, сканер (разрешение + превью), профиль/edit/share/legal/mailto, маркет → магазин → оффер → Chrome, deep link тёплый, тёмная тема, font_scale 1.3 (рост ≤15%). ❌/⚠️ → баги A7–A17 в `docs/BUGS.md`: P1 — A7 worklet-ошибка в `AnimatedGradientText`, A13 «Сообщения» вечный skeleton на пустом аккаунте, A9 PromptSheet не поднимает клавиатуру по тапу (2 из 3 попыток); P2 — A8 угол пилюли таб-бара, A10 Back закрывает весь экран ачивок вместо DetailsSheet, A12 «Оценить в App Store» на Android, A14 «3 ч» вместо «1 мин» (naive utcnow на бэке); P3 — A11, A16, A17. A15 закрыт #172. Не проверено: чат (нет диалогов у нового аккаунта), онбординг-свайпы (уже пройден), deep link из холодного старта — на release-APK.
...
- **2026-09-10** — Первый preview-APK через EAS (#188 — Sentry-upload на Android только через `SENTRY_DISABLE_AUTO_UPLOAD`), владелец гоняет на реальном realme/oppo 360dp с 3-кнопочной навигацией. Найдено и закрыто: обложки серые = незазеркаленные ссылки на `i.discogs.com`, из РФ не отвечает (#189, мост `/covers/{id}.jpg` + ретраи, задеплоено), вёрстка под 360dp и нижнюю панель (#193, хук `useBottomContentInset`). **Доставка тестерам — OTA на канал `preview-android`** (`npm run update:preview:android`), APK пересобирать только при нативных изменениях (иконка, плагины, разрешения).
- **2026-09-08 (вечер)** — Влиты #167–#172: правки по ре-ревью, WS4a+WS4 (эмулятор на маке, EAS-профили, minSdk 26, первый prebuild), баги A1–A6, клэмп шрифта на iOS (решение владельца), белый экран при логине/регистрации (дерево не размонтируется), звук канала. Владелец зарегистрировался и вошёл на эмуляторе; QA-прогон WS7 — A7–A17. **Следующий шаг:** закрыть P1/P2 из A7–A14, собрать release-APK (`eas build -p android --profile preview` или локальный `assembleRelease`) и повторить deep link/A4 на нём; дальше WS6 (ассеты) и WS8 — уже с аккаунтами Play.
- **2026-09-08** — Влиты #160–#166. Клэмп масштаба шрифта: A/B в Expo Go на iOS-симуляторе при accessibility XXXL показал, что старый `Text.defaultProps` был no-op и на iOS (заголовок логина ломался на две строки), рабочий clamp через Metro-alias (`metro.config.js` + `lib/fontScale/`) влит #166, **с 08.09 включён на обеих платформах** (решение владельца: на маленьких iPhone с крупным текстом кнопки ломались на две строки). Adversarial ре-ревью после мерджей: P1 — runbook `min-version` без `platform` (починен), остальное P2: jest не гоняет Metro-alias (нужен `moduleNameMapper` или тест на `resolveRequest`), `Animated.Text`/`BottomSheetTextInput` клэмпом не накрыты, `onChange` у 4 gorhom-шитов теперь только на Android, `snapshot-ios.sh` больше не глушит stderr introspect. Перед сборкой iOS 1.0.1: визуально сверить splash с TestFlight 1.0.0 и решить вопрос клэмпа на iOS.
