...
> Дополняет [PLAN_NOTIFICATIONS_V2.md](PLAN_NOTIFICATIONS_V2.md). Не дублирует транспорт/дедуп/snooze — переиспользует их. Здесь только то, чего в v2 нет: **per-item подписка (bell)**, **живой `wishlist_price_drop`**, **история цен**.
