...
bucket простаивает, на проде стоит агрессивный профиль **18 / 35 / 1.0**.

Почему не больше. Peek смотрит скользящее 60с-окно, а джоба тикает раз в
минуту: после жирного прогона следующий тик застаёт окно занятым и получает
бюджет ~0. Устоявшийся темп тик-схемы ≈ (55 − headroom) / 2 в минуту —
при 18 это **~18 req/min ≈ 26K обложек/день**, очередь 11.3M ≈ 14 месяцев.
Хочется быстрее — нужен непрерывный воркер вместо минутных тиков, это
отдельная задача. И пол headroom не опускать ниже 16: при free ≤
INTERACTIVE_RESERVE (15) лимитер вообще не обслуживает фоновый приоритет —
дрип ловил бы таймауты вместо ответов. Обе границы закодированы в
`config.py` (Field ge=) и закрыты тестами `test_cover_drip_profile.py`.

⚠️ **Перед релизом в App Store агрессивный профиль ОБЯЗАТЕЛЬНО снять**:
удалить три COVER_DRIP_* переменные из `.env.prod` и пересоздать scheduler
(`docker compose up -d --force-recreate scheduler` — просто `restart` может
не перечитать env-файл) — дефолты вернут щадящий режим. Пункт продублирован
в чеклистах APPSTORE_PRERELEASE_AUDIT.md и PLAN_RELEASE_v2.md.
Красная линия при наблюдении: `docker compose logs api scheduler | grep -i
429` — именно **оба** контейнера: 429 логирует тот, кто делал запрос, и
юзерские 429→503 живут в api, а не в scheduler. Эффективный профиль дрип
печатает в каждой строке лога `cover drip: ... (profile hr=.. cap=.. pace=..)`.
