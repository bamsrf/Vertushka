...
|---|---|---|---|
| 🔴 Блокер | 3 | Не сабмитить | ✅ закрыты 2026-08-14 |
| 🟠 Высокий | 3 | До открытия на публику | ⏳ |
| 🟡 Средний | 7 | Первые две недели | ⏳ |
| ⚪ Гигиена | 6 | Бэклог | ⏳ |

> **Статус блокеров.** S1, S2, S3 исправлены и закрыты регресс-тестами в
> `Backend/tests/test_security_hardening.py` (52 теста; весь набор — 524 зелёных).
> Ниже описания сохранены как есть: они объясняют, что именно было сломано и
> почему фикс выглядит так. Изменения:
...
>   добавлено удаление `uploads/user_photos/{user_id}`.
...
| `playwright` | 1.41.2 | Chromium ~Jan-2024, с тех пор десятки RCE | Косвенно, но headless-Chromium **рендерит чужие HTML** магазинов |
