...
## 🤖 Android-порт — полировка с реального телефона (2026-09-10)

Четыре замечания владельца с реального Android-телефона (~360dp, 3-кнопочная навигация), PR `fix/android-polish`. Проверка — `pixel8_api36` (Android 16), dev-client, без логина (пароля демо-аккаунта в репо нет, регистрироваться/логиниться по ТЗ нельзя), поэтому всё за логином проверено кодом. Тег `[android]`.

### A18. [android] Сердечки want-count в «Новинках» на Поиске — красные
**Файл:** `Mobile/components/AutoRail.tsx` (`♥ {r.discogs_want}` под обложкой; рейл используется на Поиске и в публичном профиле)

**Причина:** символ `♥` (U+2665) — текстовый глиф. В Roboto его нет, Android берёт фолбэк из Noto Color Emoji и рисует **цветное** эмодзи-сердце, игнорируя `color` стиля. iOS отдаёт глиф из SF/Inter и красит его в `periwinkle`, поэтому там сердечко всегда было в тон.

**Статус:** ✅ закрыт 2026-09-10 — вместо глифа `<Icon name="heart" size={ms(11)} color={Colors.royalBlue}>` (Phosphor fill), счётчик тем же `royalBlue`. **Общая правка** для обеих платформ по решению владельца («сделаем фиолетовыми, не в нашем стиле»): на iOS сердечко из periwinkle-глифа стало брендовой иконкой.

---

### A19. [android] Адаптивная иконка обрезает маскота
**Файл:** `Mobile/app.json` → `android.adaptiveIcon`, `Mobile/assets/images/adaptive-icon.png`

**Причина:** foreground 1024×1024 с маскотом почти во весь холст (bbox 174–848 × 253–770). Лаунчер показывает под маской только safe zone 66/108dp (~61% холста), тонарм и края диска срезались.

**Статус:** ✅ закрыт 2026-09-10 — новый foreground: маскот из `icon.png` (iOS-иконка не тронута), вырезан по контуру (flood-fill внешнего белого), вписан в окружность r=306px от центра холста (центр подобран минимизацией радиуса — маскот с тонармом несимметричен), поля прозрачные; ширина маскота 56% холста. `backgroundColor` остаётся `#ffffff` — фон исходной `icon.png` (углы 255/255/255; `#FAFBFF` у splash — другой ассет). Добавлен `monochromeImage` (`adaptive-icon-monochrome.png`, белый силуэт диск+тонарм) для themed icons Android 13+. `ci/check-ios-baseline.sh` зелёный без обновления baseline. Скрипт генерации — в описании PR.

---

### A20. [android] Подсказка «Прокрути вниз или нажми — и ты в Маркете» внизу Поиска не тапается
**Файл:** `Mobile/app/(tabs)/search.tsx` (curtain-CTA в `ListHeaderComponent`), `Mobile/lib/useBottomContentInset.ts`

**Причина:** до #193 нижний клиренс списка был константой 112 «под iOS»; с 3-кнопочной панелью (48dp) и пилюлей над ней CTA оставалась под `GlassTabBar`/панелью, тап уходил в них. Второй фактор — на Android `FlatList` не тянется за конец контента (нет bounce), поэтому overdrag-commit («прокрути вниз») там не срабатывает в принципе: тап — единственный вход в Маркет с этой плашки.

**Статус:** ✅ по коду (#193 + 2026-09-10) — клиренс `useBottomContentInset({ tabBar: true, extra: 24 })` = `max(inset, 16) + 12 + 60 + 24` (3 кнопки: 48 → 144dp), fade-маска считается от того же числа. Перехватчиков тапов нет: кастомный `tabBar` в expo-router-обёртке `BottomTabView` рендерится без absolute-обёртки, контейнер `GlassTabBar` — 65% ширины по центру, fade-градиент `pointerEvents="none"`. На AVD не прогнано (Поиск за логином) — **проверить на телефоне владельца**: доскроллить вниз, плашка целиком над пилюлей, тап открывает Маркет. Текст «Прокрути вниз» на Android формально неверен (жеста нет) — оставлен, по решению владельца можно заменить через `Platform.select`.

---

### Как работает «Назад» на Android

- **Источник события:** кнопка панели или жест от края → `MainActivity` → `ReactActivity` (RN 0.86 регистрирует `OnBackPressedCallback`, потому что для targetSdk 36 `onBackPressed()` больше не зовётся напрямую) → JS `BackHandler` → react-navigation. iOS-свайпа «от левого края» у native-stack на Android нет: `gestureEnabled` в `app/_layout.tsx` — только iOS.
- **Стек:** один корневой `Stack` (`app/_layout.tsx`), все экраны (`record/[id]`, `artist/[id]`, `collection/value`, `radar`, `achievements`, `profile`, `settings/*`, legal) лежат в нём → системный «Назад» = `pop` на предыдущий экран, тот же путь, что `router.back()` в их шапках. Если стек пуст (холодный старт по deep link) — `router.back()` уводит на `(tabs)`.
- **Табы:** `backBehavior` дефолтный (`firstRoute`): «Назад» со Скана/Коллекции → Поиск, с Поиска → приложение сворачивается (`exitApp`). Не баг, зафиксировано в наблюдениях WS7.
- **Шиты и оверлеи:** RN `Modal` закрывается нативно (`onRequestClose`); gorhom-шиты и inline-оверлеи — `useAndroidBackClose(open, close)` (`OffersBottomSheet`, `PriceHistorySheet`, `ThresholdSheet`, `achievements.tsx` A10). Новый шит без хука будет уводить с экрана целиком — правило для ревью.
- **`predictiveBackGestureEnabled` (Q7 плана) — оставлен выключенным.** Проверено 2026-09-10 сборкой с флагом (`android:enableOnBackInvokedCallback="true"`, жестовая панель, legal-страница): жест от края показывает только системную стрелку «назад», **превью предыдущего экрана не появляется** — `react-native-screens` 4.26 в обычном (не `gamma`) стеке не регистрирует `OnBackPressedCallback` с прогрессом, pop идёт через JS `BackHandler` → react-navigation, как и без флага; на отпускание стек корректно уходит на регистрацию, ничего не ломается. Т.е. включение не даёт видимого «заглянуть назад», а лишь меняет системный dispatch на Android 13–15 (RN 0.86 свой колбэк регистрирует при targetSdk 36 в любом случае), при этом сторонние Activity (crop expo-image-picker, dev-launcher) и шиты за логином на этом пути не прогнаны. Включать, когда expo-router перейдёт на стек с нативным predictive back (screens `gamma`), и после прогона за логином на устройстве.

**Итог проверки 2026-09-10 (pixel8_api36, 3-кнопочная панель, тап по кнопке «Назад»):** legal-страница из регистрации → назад на форму регистрации с сохранённым состоянием ✅; «Забыли пароль» → назад на вход ✅; «Назад» на корне (вход) → приложение сворачивается на лаунчер, не крашится ✅. Экраны за логином (`record/[id]`, `artist/[id]`, `collection/value`, `radar`, `achievements`, `profile`, настройки) на AVD недоступны без аккаунта — по коду они в том же стеке, «Назад» для них идентичен legal-странице; прогнать на телефоне владельца.
