...
export default function SearchScreen() {
  ...
  // Выход из Маркета может случиться не здесь. Экран /market — отдельный
  // root-route поверх табов, и его pull-to-search снимает себя со стека,
  // возвращая человека на ЭТОТ, уже смонтированный, экран (так он не
  // пересобирается и не перезапрашивает всё заново). Но локальный `committed`
  // от этого не меняется: если слой был поднят до ухода на карточку релиза,
  // Поиск встретил бы человека тем самым Маркетом, из которого он только что
  // вышел. Подписка на стор — единственный канал, по которому чужой выход
  // доезжает до живого экрана.
  //
  // Только в сторону закрытия: поднимать слой обязаны жест и ?focus=market,
  // иначе стор открывал бы Маркет сам, без действия человека.
  const storeCommitted = useMarketStore((s) => s.committed);
  useEffect(() => {
    if (storeCommitted) return;
    // Подписка видит значение того рендера, в котором эффект запланирован, а
    // локальный `committed` уезжает в стор соседним эффектом — то есть в кадре
    // собственного открытия Маркета здесь лежит ещё false при уже поднятом
    // слое. Сверяемся с живым стором, иначе свой же commit тут же схлопнулся
    // бы обратно.
    if (useMarketStore.getState().committed) return;
    setCommitted(false);
    committedAnim.value = withSpring(0, SPRING_CONFIG);
  }, [storeCommitted, committedAnim]);
