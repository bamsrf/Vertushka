...
/**
 * Системный Reduce Motion сам по себе.
 *
 * Нужен там, где анимацию нельзя просто выключить, но можно смягчить:
 * жест-подсказка (lib/useGestureNudge.ts) — единственный способ узнать о
 * невидимом жесте, поэтому под Reduce Motion она не исчезает, а становится
 * короче и линейнее. Гейты выше отвечают на вопрос «крутить ли вообще» и для
 * такого выбора не годятся: они складывают настройку с фокусом и фоном.
 */
export function useReduceMotion(): boolean {
  return useGateStore((s) => s.reduceMotion);
}
