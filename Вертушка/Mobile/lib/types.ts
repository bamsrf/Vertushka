...
/**
 * Откуда начался путь в магазин. Обязательный аргумент `api.trackOfferClick` —
 * именно по нему строится разбивка в отчёте магазину («60% переходов из
 * Маркета»), поэтому забыть его нельзя: тип не даст.
 *
 * Читается как «ближайшая точка входа», а не «какая кнопка нажата»:
 *   • `wishlist_swipe`, `wishlist_digest`, `radar_price_history` — покупка
 *     происходит прямо там, поэтому точка входа и есть сам элемент;
 *   • `market`, `market_store` — кнопка «Купить» физически на карточке
 *     пластинки, но привёл туда Маркет, и в отчёте засчитывается он. Иначе
 *     вклад Маркета навсегда прятался бы внутри `record` (так и было: обе
 *     метки лежали в словаре, но их никто не проставлял);
 *   • `record` — фолбэк «пришли как-то иначе»: из поиска, коллекции,
 *     диплинка. Не «карточка пластинки как место», а «происхождение неясно».
 *
...
/**
 * Точки входа, которые карточка пластинки принимает в query-параметре `from`
 * и потом проставляет в клик по «Купить».
 *
 * Валидируем список, а не берём параметр как есть: `from` приходит из URL, а
 * значит его может подставить кто угодно (диплинк, ссылка из мессенджера) —
 * и мусор осел бы в `offer_clicks.source`, откуда мы строим отчёт магазину.
 */
const RECORD_ENTRY_SOURCES = ['market', 'market_store'] as const;

/**
 * Разобрать `from` из навигации. Всё незнакомое (включая отсутствие
 * параметра — прямой заход, поиск, коллекция, диплинк) — это `record`.
 */
export function parseEntrySource(raw: string | string[] | undefined): ClickSource {
  const value = Array.isArray(raw) ? raw[0] : raw;
  return (RECORD_ENTRY_SOURCES as readonly string[]).includes(value ?? '')
