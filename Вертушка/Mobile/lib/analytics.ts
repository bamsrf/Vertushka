...
export const analytics = {
  ...
  /**
   * Чем кончился переход «В Маркет» с карточки релиза.
   *
   * Ради этой цифры фича и делалась: до неё было видно только, что человек
   * зашёл в Маркет, и было не отличить «пришёл и нашёл свою пластинку» от
   * «пришёл, не увидел её и ушёл, решив, что кнопка соврала». `outcome`
   * делит вход надвое, `results_count` показывает, сколько всего нашлось
   * вместе с другими прессингами.
   *
   * Шлём ПОСЛЕ ответа: до него исход неизвестен, а догадка здесь — это ровно
   * то враньё, которое фича и убирает.
   */
  marketReleaseScope: (params: {
    outcome: 'in_stock' | 'empty';
    results_count: number;
  }) => track('market_release_scope', params),

  /**
   * Сняли сужение по релизу — крестиком на плашке или начав новый поиск.
   * Вместе с market_release_scope отвечает, работает ли витрина как запасной
   * ответ на «этой нет»: если сбрасывают часто, люди идут смотреть дальше.
   */
  marketReleaseReset: (params: { reason: 'chip' | 'search' }) =>
    track('market_release_reset', params),
