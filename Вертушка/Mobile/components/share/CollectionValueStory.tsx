 ...
 */
function MonoNumber({ value, size, color, prefix, suffix, floatSuffix }: {
  value: number;
  size: number;
  color: string;
  prefix?: string;
  ...
  /**
   * Выносит ₽ из потока. Тогда центрируются сами цифры, и зазор между
   * группами тысяч встаёт ровно на ось контейнера — на лейбле это ось дырки.
   */
  floatSuffix?: boolean;
}) {
  ...
  return (
    <View style={{ flexDirection: 'row', alignItems: 'baseline' }}>
      ...
      {suffix ? (
        <View style={floatSuffix ? { position: 'absolute', left: '100%', bottom: 0, marginLeft: size * 0.22 } : { marginLeft: size * 0.22 }}>
          {suffix}
        </View>
      ) : null}
    </View>
    ...
    <View style={[s.base, { backgroundColor: C.deep }]}>
      ...
      <View style={s.safeColumn}>
        ...
        <View style={{ gap: 6 }}>
          ...
          <MonoNumber value={data.totalRub} size={34} color={C.ivory} suffix={<Rub size={23} color={C.ember} />} />
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
            {data.deltaRub != null && data.deltaRub !== 0 && (
              <View style={s.deltaPill}>
                <Icon name="arrow-up-right" size={9} color={C.green} />
        ...
        <View style={{ gap: 8 }}>
          ...
          {first && (
            <View style={[s.card, s.cardFirst]}>
              <Text style={[s.rank, { color: C.ember, fontSize: 17 }]}>1</Text>
              <Cover uri={first.coverUrl} size={36} radius={6} />
              <View style={{ flex: 1, gap: 2 }}>
                ...
                {first.isCollectible && (
                  <View style={s.goldPill}>
                    <Icon name="star" size={6.5} color={C.gold} />
                    <Text style={s.goldText}>КОЛЛЕКЦИОНКА</Text>
                  </View>
                )}
              </View>
              <MonoNumber value={first.priceRub} size={12} color={C.ivory} prefix="≈" suffix={<Rub size={12} color={C.ivory} />} />
            </View>
          )}
          {rest.map((r, i) => (
            <View key={`${r.title}-${i}`} style={s.card}>
              <Text style={[s.rank, { color: C.cobaltSoft }]}>{i + 2}</Text>
              <Cover uri={r.coverUrl} size={33} radius={5} />
              <View style={{ flex: 1, gap: 2 }}>
                <Text numberOfLines={1} style={s.cardTitle}>{r.title}</Text>
                <Text numberOfLines={1} style={s.cardArtist}>{r.meta}</Text>
              </View>
              <MonoNumber value={r.priceRub} size={12} color={C.ivory} prefix="≈" suffix={<Rub size={12} color={C.ivory} />} />
            </View>
          ))}
        </View>
        ...
        {/* Подвал */}
        <View style={[s.rowBetween, { alignItems: 'flex-end', minHeight: 64 }]}>
          <View style={{ gap: 7, paddingBottom: 5, flexShrink: 1 }}>
            ...
            <LinearGradient colors={[C.cobalt, '#5C7AE8']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={s.linkPill}>
              ...
              <Icon name="arrow-right" size={11} color="#FFFFFF" />
            </LinearGradient>
          </View>
          <Image source={MASCOT} style={{ width: 66, height: 66, marginRight: -7 }} contentFit="contain" />
        </View>
      </View>
    </View>
  );
    ...
    <View style={[s.base, { backgroundColor: C.ivory }]}>
      ...
      <View style={s.tag}>
        ...
        <View style={{ gap: 3 }}>
          ...
          <MonoNumber value={data.totalRub} size={34} color={C.navyText} suffix={<Rub size={23} color={C.emberLight} />} />
        ...
        <View style={{ gap: 9 }}>
          ...
          {/* Линейка — общая для всех строк: выноски переменной длины между
              названием и ценой читались как разные по смыслу. */}
          {data.top.map((r, i) => (
            <View key={`${r.title}-${i}`} style={s.priceRow}>
              <Text numberOfLines={1} style={[s.rank, { color: i === 0 ? C.emberLight : C.cobalt, width: 22, textAlign: 'left', fontSize: 12 }]}>0{i + 1}</Text>
              <View style={{ borderWidth: 0.7, borderColor: C.navyText, borderRadius: 4 }}>
                <Cover uri={r.coverUrl} size={34} radius={3} />
              </View>
              <View style={{ flex: 1, minWidth: 0, gap: 1 }}>
                <Text numberOfLines={1} style={[s.cardTitle, { color: C.navyText, fontSize: 13.5 }]}>{r.title}</Text>
                <Text numberOfLines={1} style={[s.cardArtist, { color: C.ivoryMuted, fontSize: 9.5 }]}>{r.meta}</Text>
              </View>
              <MonoNumber value={r.priceRub} size={12.5} color={C.navyText} suffix={<Rub size={12.5} color={C.navyText} />} />
            </View>
          ))}
        </View>
        ...
        <View style={[s.rowBetween, { alignItems: 'flex-end' }]}>
          <View style={{ gap: 4 }}>
            ...
            <Text numberOfLines={1} style={{ fontFamily: 'Inter_600SemiBold', fontSize: 9.5, color: C.navyText }}>{LINK}</Text>
          </View>
  ...
  return (
    <View style={[s.base, { backgroundColor: C.navy }]}>
      ...
      <View style={[s.labelCircle, { width: LABEL, height: LABEL, borderRadius: LABEL / 2, left: (STORY_BASE_WIDTH - LABEL) / 2, top: DISC_TOP + (DISC - LABEL) / 2 }]}>
        <View style={{ position: 'absolute', left: 0, right: 0, top: 0, height: LABEL / 2, alignItems: 'center', justifyContent: 'flex-end', paddingBottom: HOLE / 2 + 13, gap: 3 }}>
          <LogoMark size={16} dark={false} />
          <Text numberOfLines={1} style={[s.overline, { fontSize: 9, letterSpacing: 1.1, color: C.emberLight }]}>@{data.username.toUpperCase()}</Text>
          <View style={{ marginTop: 2 }}>
            <MonoNumber value={data.totalRub} size={21} color={C.navyText} suffix={<Rub size={14} color={C.cobalt} />} floatSuffix />
          </View>
        </View>
        ...
        <View style={{ position: 'absolute', left: 0, right: 0, top: LABEL / 2, height: LABEL / 2, alignItems: 'center', paddingTop: HOLE / 2 + 11, gap: 2 }}>
          {data.deltaRub != null && data.deltaRub !== 0 && (
            <Text numberOfLines={1} style={{ fontFamily: 'Inter_700Bold', fontSize: 10, color: C.greenDark }}>
              {data.deltaRub > 0 ? '+' : '−'}{formatRubStory(Math.abs(data.deltaRub))} ₽ за месяц
            </Text>
          )}
          <Text numberOfLines={1} style={{ fontFamily: 'Inter_500Medium', fontSize: 8.5, color: C.ivorySecondary }}>
            {data.recordsCount} {pluralRecords(data.recordsCount)}{data.oldestYear ? ` · с ${data.oldestYear}` : ''} · 33 ⅓ RPM
          </Text>
          <Text numberOfLines={1} style={{ marginTop: 3, fontFamily: 'Inter_600SemiBold', fontSize: 8, letterSpacing: 0.4, color: C.cobalt }}>{LINK}</Text>
        </View>
      </View>

      <Text numberOfLines={1} style={[s.overline, { position: 'absolute', left: 24, right: 24, top: 56, fontSize: 12, letterSpacing: 1.6 }]}>МОЯ КОЛЛЕКЦИЯ СТОИТ</Text>

      {/* Топ-3 на всю ширину: дизайн важнее правой TikTok-зоны (решение владельца). */}
      <View style={{ position: 'absolute', left: 24, right: 24, top: LIST_TOP, gap: 9 }}>
        <Text numberOfLines={1} style={[s.sectionLabel, { fontSize: 11, letterSpacing: 1.3, color: C.cobaltSoft, marginBottom: 2 }]}>САМЫЕ ДОРОГИЕ</Text>
        {data.top.map((r, i) => (
          <View key={`${r.title}-${i}`} style={[s.card, { paddingVertical: 9, paddingLeft: 8, paddingRight: 11, gap: 9 }]}>
            <Text numberOfLines={1} style={[s.rank, { color: i === 0 ? C.ember : C.cobaltSoft, fontSize: 17, width: 19 }]}>{i + 1}</Text>
            <Cover uri={r.coverUrl} size={36} radius={6} />
            <View style={{ flex: 1, gap: 2 }}>
              <Text numberOfLines={1} style={[s.cardTitle, { fontSize: 15.5 }]}>{r.title}</Text>
              <Text numberOfLines={1} style={[s.cardArtist, { fontSize: 11.5 }]}>{r.meta}</Text>
            </View>
            <MonoNumber value={r.priceRub} size={13} color={i === 0 ? C.ember : C.ivory} prefix="≈" suffix={<Rub size={13} color={i === 0 ? C.ember : C.ivory} />} />
          </View>
        ))}
      </View>
    </View>
...
const s = StyleSheet.create({
  ...
  handleText: { fontFamily: 'Inter_600SemiBold', fontSize: 9.5, color: C.textSecondary },
  overline: { fontFamily: 'Inter_700Bold', fontSize: 10, letterSpacing: 1.3, color: C.cobaltSoft },
  deltaPill: {
  ...
  deltaText: { fontFamily: 'Inter_700Bold', fontSize: 9.5, color: C.green },
  usdText: { fontFamily: 'Inter_500Medium', fontSize: 10, color: C.textSecondary },
  sectionLabel: { fontFamily: 'Inter_700Bold', fontSize: 9.5, letterSpacing: 1, color: C.textMuted },
  card: {
    flexDirection: 'row', alignItems: 'center', gap: 8, paddingVertical: 8, paddingLeft: 7, paddingRight: 9,
    borderRadius: 10, backgroundColor: C.elevated, borderWidth: 0.7, borderColor: C.border,
  },
  cardFirst: { backgroundColor: 'rgba(58,31,18,0.85)', borderColor: 'rgba(255,122,74,0.35)' },
  rank: { fontFamily: 'RubikMonoOne-Regular', fontSize: 14, width: 16, textAlign: 'center' },
  cardTitle: { fontFamily: 'Inter_700Bold', fontSize: 13, color: C.text },
  cardArtist: { fontFamily: 'Inter_500Medium', fontSize: 10, color: C.textSecondary },
  goldPill: {
  ...
  goldText: { fontFamily: 'Inter_700Bold', fontSize: 7.5, letterSpacing: 0.4, color: C.gold },
  tile: { flex: 1, gap: 2, paddingVertical: 7, paddingHorizontal: 7, borderRadius: 8 },
  tileDark: { backgroundColor: C.elevated, borderWidth: 0.7, borderColor: C.border },
  tileValue: { fontFamily: 'RubikMonoOne-Regular', fontSize: 15, includeFontPadding: false },
  tileLabel: { fontFamily: 'Inter_500Medium', fontSize: 8.5 },
  cta: { fontFamily: 'Inter_800ExtraBold', fontSize: 13, letterSpacing: -0.3, color: C.text },
  linkPill: { flexDirection: 'row', alignItems: 'center', alignSelf: 'flex-start', gap: 4, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 999 },
  linkText: { fontFamily: 'Inter_600SemiBold', fontSize: 10, color: '#FFFFFF' },
  ...
  ivoryHandle: { fontFamily: 'Inter_600SemiBold', fontSize: 9, color: C.ivorySecondary },
  ivorySub: { fontFamily: 'Inter_500Medium', fontSize: 10, color: C.ivorySecondary },
  ...
  priceRow: {
    ...
    borderBottomWidth: 0.7, borderBottomColor: '#D6DAE4',
  },
  stamp: {
    alignSelf: 'flex-start', paddingHorizontal: 7, paddingVertical: 3, borderWidth: 1, borderColor: C.emberLight, borderRadius: 4,
    transform: [{ rotate: '-4deg' }],
  },
  stampText: { fontFamily: 'Inter_800ExtraBold', fontSize: 9.5, letterSpacing: 1, color: C.emberLight },
