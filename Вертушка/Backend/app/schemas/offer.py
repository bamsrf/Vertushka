...
#: Откуда внутри UI юзер ушёл в магазин.
#:
#: Осознанно НЕ Literal и НЕ CHECK-констрейнт. Literal вернул бы 422 на
#: незнакомом значении, а порядок деплоя у нас «бэкенд → сборка мобилки»: новая
#: сборка с новым источником против ещё не обновлённого бэкенда потеряла бы не
#: только аналитику, но и саму возможность купить. Незнакомые значения
#: сворачиваем в `unknown` и пишем warning — так ломается отчёт, а не покупка.
KNOWN_CLICK_SOURCES = frozenset(
    {
        ...
        "wishlist_digest",  # поп-ап «Где купить» из дайджеста уведомлений
        "radar_price_history",  # sheet истории цен в Радаре
        "web_profile",      # публичная веб-страница профиля
        "unknown",          # сборки, которые source ещё не присылают
    }
)


class OfferClickRequest(BaseModel):
    """Body для POST /api/offers/{id}/click.

    Целиком опционален: старые сборки шлют пустой POST, и это не должно ломать
    переход в магазин — они просто попадут в отчёты как `unknown`.
    """
    ...
    source: str = Field(
        ...
        max_length=24,
        description=(
            "Место в UI, откуда нажали «Купить» — для разбивки в отчёте магазину "
            "(«60% переходов из Маркета») вместо одного общего числа. "
            f"Известные значения: {', '.join(sorted(KNOWN_CLICK_SOURCES))}. "
            "Незнакомое значение не отбивается ошибкой, а записывается как "
            "`unknown`: терять покупку из-за метки аналитики нельзя."
        ),
    )


class OfferClickResponse(BaseModel):
    """Ответ POST /api/offers/{id}/click — URL для Linking.openURL."""

    click_id: UUID
    url: str = Field(
        ...,
        description=(
            "Ссылка нашего редиректора вида {PUBLIC_BASE_URL}/go/{click_id}, "
            "а НЕ ссылка магазина. Открывать как есть: редиректор отдаст 302 на "
            "магазин, собрав affiliate-обёртку с subid=click_id, подтвердит "
            "переход и отправит хит в Метрику. Не разворачивать и не подменять "
            "на preview_url — иначе переход выпадет из аттрибуции."
        ),
    )
