...
# Это НЕ порог принятия (тот — score >= 1.4 из двух отдельных similarity),
# а только ширина перебора: чем он ниже, тем больше кандидатов достаёт GIN.
_DUMP_TRGM_THRESHOLD = 0.4
...
async def _lookup_in_dump_index(
    ...
    *,
    ...
    if artist and title:
        # Порог перебора кандидатов. По умолчанию pg_trgm отдаёт всё от 0.3, и
        # на широком артисте это миллионы строк: «Various Artists» + «The Sound
        # Of Jazz» = 995k × 830k, запрос идёт 60.9 c и не укладывается в
        # statement_timeout (30 c). Тогда он отменяется, листинг проваливается
        # на платный шаг, и Discogs находит релиз, лежащий в нашем же дампе —
        # так все 5 663 «находки» API оказались уже у нас (замеры 12.08).
        #
        # 0.4 — не подобранное число, а доказуемая граница: принимаем score
        # >= 1.4 из двух similarity, каждая максимум 1.0, значит по каждому полю
        # нужно минимум 0.4. Кандидат ниже 0.4 не мог быть принят никогда, и
        # отсечь его безопасно по построению. Замер: 60.9 c → 20.7 c.
        #
        # SET LOCAL — порог живёт до конца транзакции и не протекает в соседние
        # запросы через пул соединений (в отличие от session-level set_limit()).
        await db.execute(
            text(f"SET LOCAL pg_trgm.similarity_threshold = {_DUMP_TRGM_THRESHOLD}")
        )
