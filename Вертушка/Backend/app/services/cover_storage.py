...
class CoverStorageService:
    ...
    async def download_and_store(
        ...
        *,
        ...
        # Инициализация ДО ветки: ниже, в пути скачивания, обе переменные
        # читаются для replace_floor. Когда файла на диске нет (а это основной
        # случай — первая запись зеркала), ветка `if dest.exists()` не
        # выполняется, и обращение к needs_upgrade падало UnboundLocalError.
        # Исключение ловил общий except вокруг скачивания, поэтому наружу это
        # выглядело как «не удалось скачать обложку» — ни одного нового зеркала
        # с 15.09.2026, при том что сама картинка уже была скачана.
