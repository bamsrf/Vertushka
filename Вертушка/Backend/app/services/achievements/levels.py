...
async def compute_score(db: AsyncSession, user_id: UUID) -> int:
    ...
    в момент, когда юзер её взял. Поэтому переоценка тира прошлое не
    переписывает. Уменьшиться сумма может только одним способом — отзывом
    revocable-ачивки (K8–K10: удалённый ручной релиз перестаёт быть вкладом).
