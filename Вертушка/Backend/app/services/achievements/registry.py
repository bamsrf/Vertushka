...
class AchievementDefinition:
    ...
    # Ачивка держится на состоянии, которое юзер может откатить сам, — тогда
    # анлок не вечен. Такие определения evaluator прогоняет даже по уже
    # открытой ачивке и снимает её, если условие перестало выполняться
    # (progress при этом умеет уменьшаться). По умолчанию False: «сделал
    # однажды» отбирать нельзя.
