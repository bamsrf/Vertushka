...
# ---- Семья носителя: Python + зеркало в SQL ----------------------------- #

#: Формат из `infer_format` → «семья носителя». Box Set намеренно отсутствует:
#: бокс бывает и виниловый, и CD, так что семьи у него нет и конфликт по нему
#: не доказывается. Используется и `listing_matcher._format_family`, и
#: `sql_format_family` — одна таблица на оба пути.
FORMAT_FAMILY: dict[str, str] = {
    "LP": "VINYL",
    "2xLP": "VINYL",
    "EP": "VINYL",
    "Single": "VINYL",
...
def _to_pg_regex(pattern: re.Pattern) -> str:
    r"""Python-regex → Postgres ARE, пригодный для подстановки в SQL-строку.

    Три замены:
      * `\b` → `\y` — граница слова в ARE пишется иначе;
      * `'` → `''` — экранирование внутри SQL-литерала;
      * `(?:` → `(` — и вот это не косметика. SQLAlchemy `text()` разбирает
        `:имя` как bind-параметр, поэтому «(?:lp|vinyl)» внутри регулярки
        превращался в требование передать параметр `lp`, и запрос падал с
        «A value is required for bind parameter 'lp'». Захват группы нам не
        нужен ни в одном паттерне, так что `(?:` безопасно становится `(`.
        Инвариант «в готовом SQL нет ни одного двоеточия» закреплён тестом —
        новый паттерн с `:` упадёт на нём, а не ночью в проде.
    """
    return (
        pattern.pattern
        .replace(r"\b", r"\y")
...
def sql_format_family(col: str) -> str:
    """SQL-выражение «семья носителя» для колонки — зеркало `infer_format`.

    ГЕНЕРИРУЕТСЯ ИЗ `_FORMAT_MAP`, а не пишется руками: у соседнего
    `sql_color_family` мирроринг ручной, и там в docstring стоит «держать в
    синхроне» — то есть ровно то место, где расхождение однажды случится
    молча. Здесь порядок веток CASE повторяет порядок паттернов, а первое
    совпадение выигрывает — как и в цикле `infer_format`.

    Зачем SQL вообще: `rematch_format_conflicts_batch` искал конфликты в
    Python, а из базы брал 500 строк по `matched_at ASC`. Чистые строки из
    окна не уходили никогда (их `matched_at` не меняется), поэтому джоба
    каждую ночь смотрела одни и те же 500 майских листингов: 2 конфликта из
    500 при 1 645 на проде. С предикатом в SQL `LIMIT` считает конфликты, а не
    просмотренные строки.
    """
    whens = " ".join(
        f"WHEN {col} ~* '{_to_pg_regex(pattern)}' THEN "
        + (f"'{FORMAT_FAMILY[fmt]}'" if fmt in FORMAT_FAMILY else "NULL")
        for pattern, fmt in _FORMAT_MAP
    )
...
#: по нему «да, цветной».
COLORED_UNSPECIFIED = "coloured"

#: Маркер общего «цветного» — только вплотную к носителю. Без этого «Post Self
#: Coloured» (название альбома) стало бы цветным винилом.
_COLORED_MARKER_RE = re.compile(
...
def infer_vinyl_color(
    ...
    *,
    ...
    require_cue=True отключает свободный третий проход и оставляет только цвет
    рядом со словом-носителем. Нужен там, где вырезать название нечем: раннер
    зовёт функцию на `title_raw`, а это и есть название альбома — вырежи его, и
    искать станет негде, а не вырежи — и «Blue Train» приедет синим винилом.
    """
    if not text:
        return None
    if exclude:
    ...
    # уходили с пустым цветом, и фильтр «Цветной винил» их не видел вовсе.
    #
    # Стоит ПОСЛЕ конкретных цветов и ПЕРЕД общим fallback'ом: «оранжевый
    # цветной винил» должен остаться оранжевым, а «Fear Of A Black Planet
