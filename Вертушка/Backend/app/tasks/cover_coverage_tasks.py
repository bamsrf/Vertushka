...
async def report_cover_coverage() -> dict:
    ...
    async with async_session_maker() as db:
        ...
        # ⚠️ Выборка идёт по cover_cached_at, а НЕ по cover_local_path.
        #
        # С включённым S3 (28.08.2026) LRU-эвикция стирает cover_local_path, но
        # оставляет cover_cached_at — файл никуда не делся, он в бакете. Пока
        # метрика смотрела на указатель локального файла, она считала долю не по
        # базе зеркал, а по тому, что СЕЙЧАС лежит в горячем кэше. А кэш после
        # перехода на двухчасовую LRU состоит преимущественно из свежезаписанного
        # — то есть из магазинных обложек, которые как раз и бывают мелкими.
        #
        # 15.09.2026 расхождение было двукратным по смыслу: 39.6% мелких по
        # старой выборке (9 238 строк) против 27.8% по всей базе зеркал
        # (39 987 строк). Алерт «доля выросла на 1.7 п.п.» отражал вымывание
        # крупных обложек из кэша, а не ухудшение качества — и продолжал бы
        # расти сам по себе, без единой новой плохой обложки.
        #
        # Без S3 поведение прежнее: там эвикция гасит и cover_cached_at, так что
        # предикат эквивалентен старому.
        tier_row = (await db.execute(text(
            ...
            "FROM records WHERE cover_cached_at IS NOT NULL"
        ))).mappings().one()
