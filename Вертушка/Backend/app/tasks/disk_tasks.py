...
Заодно считаем, сколько занимают обложки, и сравниваем с лимитом горячего
кэша: с 28.08.2026 все они живут в бакете Beget, а на диске лишь копии под
COVERS_MAX_CACHE_MB. Выше лимита — значит LRU не справляется; в пределах —
место ест Postgres или docker, и сообщение это подсказывает.
"""
...
async def check_disk_space() -> dict:
    ...
    covers_gb = _dir_size_gb(Path(settings.covers_dir))
    ...
    snapshot = {
        ...
        "used_pct": used_pct,
        "covers_gb": covers_gb,
        "covers_cap_gb": round(settings.covers_max_cache_mb / 1024, 2),
    }
...
def _covers_hint(s: dict) -> str:
    """Строка про обложки: они на диске — кэш, а не хранилище.

    С 28.08.2026 все обложки живут в бакете Beget, диск держит только горячие
    копии под лимитом COVERS_MAX_CACHE_MB, который соблюдает LRU. Старый совет
    «лечится переездом в объектное хранилище» после переезда вводил в
    заблуждение: переезжать некуда, а настоящая причина роста — что LRU не
    успевает между прогонами или не запускался вовсе (перезапуски scheduler'а
    сбивают его интервал). Поэтому сравниваем факт с лимитом.
    """
    cap, got = s["covers_cap_gb"], s["covers_gb"]
    base = (f"обложки на диске — только кэш ({got} из лимита {cap} ГБ), "
            "все файлы лежат в бакете Beget")
    if got > cap * 1.1:
        return (f"{base}. Выше лимита — LRU не успевает или не запускался: "
                "проверь «LRU cleanup» в логах scheduler; удалять руками не нужно;")
    return f"{base}. В пределах лимита — место ест не они;"
