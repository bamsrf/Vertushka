    ...
    if os.environ.get("IS_SCHEDULER", "false").lower() == "true":
        ...
        if AsyncIOScheduler is not None:
          try:
            ...
            # Раз в 2 часа, а не раз в сутки в 03:00. Суточный крон означал
            # пилу: ночью кэш подрезали до лимита, а к вечеру фоновые джобы
            # (drip раз в минуту, Deezer-бэкфилл раз в 2 минуты, апгрейд
            # качества) наливали его обратно выше лимита — замер 14.09.2026:
            # 03:00 освобождено 3.39 ГБ, к 22:50 снова 5.59 ГБ при лимите 4 ГБ,
            # то есть диск больше половины суток жил за пределом. Частый прогон
            # держит кэш у лимита; стоит он дёшево (выборка + unlink, прошлый
            # прогон на 34 860 файлов занял 9 секунд).
            #
            # Безопасно только вместе с чтением из бакета (nginx /covers/ →
            # imgproxy → s3): до него выселенная обложка требовала обратной
            # закачки на диск, и учащение чистки лишь умножило бы трафик.
            #
            # По часам, а НЕ интервалом (было 'interval', hours=2). Интервал
            # отсчитывается от старта процесса, и каждый перезапуск scheduler'а
            # (autoheal, деплой) начинал его заново: 17→18.09 было 6 перезапусков,
            # с 00:39 до 07:05 UTC чистка не прошла ни разу, на диск набежало
            # 2.5 ГБ и сработал алерт. Расписание по часам перезапуск не сбивает.
            # Раз в 15 минут: фон зеркалит ~2 100 обложек/ч (~190 МБ), и между
            # прогонами копится ~50 МБ вместо гигабайта. Минуты смещены с
            # :00/:15/…, где стоят другие джобы. misfire_grace_time: без него
            # APScheduler молча пропускает прогон, если цикл занят дольше секунды.
            scheduler.add_job(cleanup_covers, 'cron', minute='7,22,37,52',
                              id='covers_lru_cleanup', max_instances=1, coalesce=True,
                              misfire_grace_time=600)
            # Перегрев мелких мастеров в 3:40 — историческое «после ночной
            # LRU-очистки»: та освобождала место, которое апгрейд тут же
            # займёт (мелкий файл ~10 КБ → нормальный ~84 КБ). С интервальной
            # LRU привязка ко времени потеряла смысл, но час оставлен: прогон
            # на полчаса уместнее ночью, чем в пик.
