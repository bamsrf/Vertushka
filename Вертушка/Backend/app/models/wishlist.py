...
class WishlistItem(Base):
    ...
    # Режим «дешевле обычного»: скидка в % от скользящей базы (медиана дневных
    # минимумов за 90 дней), пересчитываемой на каждой проверке. Задан → порог
    # относительный и price_threshold_rub игнорируется; None → абсолютный порог.
    # Нужен потому, что абсолютный порог протухает: рынок дорожает, а число
    # остаётся, и радар молчит годами. Проценты, не доли: 20 = «на 20% дешевле».
    threshold_pct: Mapped[int | None] = mapped_column(
        Integer,
        nullable=True,
    )
